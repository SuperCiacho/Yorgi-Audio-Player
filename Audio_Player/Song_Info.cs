...
{
    public partial class Song_Info : Form
    {
        ...
        public Song_Info(string path, FMOD.System system, FMOD.Sound sound)
        {
            InitializeComponent();
            system.createSound(path, FMOD.MODE.OPENONLY, ref sound);
            ...
            for (;;)
            {
                ...
                {
                    if (tag.type == FMOD.TAGTYPE.ID3V1 || tag.type == FMOD.TAGTYPE.ID3V2)
                    {
                        #region ID3 Tags
                        switch (tag.name)
                        {
                            case "TITLE":
                                TitleBox_ID3v1.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "ALBUM":
                                AlbumBox_ID3v1.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "ARTIST":
                                ArtistBox_ID3v1.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TRACK":
                                TrackBox_ID3v1.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "YEAR":
                                YearBox_ID3v1.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "COMMENT":
                                CommentBox_ID3v1.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "GENRE":
                                GenreBox_ID3v1.Text = Genre_Determination(Marshal.PtrToStringAnsi(tag.data));                           
                                break;
                            case "TIT2":
                                TitleBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TPE1":
                                ArtistBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TALB":
                                AlbumBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TRCK":
                                TrackBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TYER":
                                YearBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TCON":
                                GenreBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "COMM":
                                CommentBox_ID3v2.Text = Marshal.PtrToStringAnsi(tag.data); break;
                        }

                        if (TitleBox_ID3v1.Text != "")
                            Title_Box.Text = TitleBox_ID3v1.Text;
                        else
                            Title_Box.Text = TitleBox_ID3v2.Text;

                        if (ArtistBox_ID3v1.Text != "")
                            Artist_Box.Text = ArtistBox_ID3v1.Text;
                        else
                            Artist_Box.Text = ArtistBox_ID3v2.Text;

                        if (AlbumBox_ID3v1.Text != "")
                            Album_Box.Text = AlbumBox_ID3v1.Text;
                        else
                            Album_Box.Text = AlbumBox_ID3v2.Text;

                        if (GenreBox_ID3v1.Text != "")
                            Genre_Box.Text = GenreBox_ID3v1.Text;
                        else
                            Genre_Box.Text = GenreBox_ID3v2.Text;

                        if (CommentBox_ID3v1.Text != "")
                            Comment_Box.Text = CommentBox_ID3v1.Text;
                        else
                            Comment_Box.Text = CommentBox_ID3v2.Text;

                        if (TrackBox_ID3v1.Text != "")
                            Track_Box.Text = TrackBox_ID3v1.Text;
                        else
                            Track_Box.Text = TrackBox_ID3v2.Text;

                        if (YearBox_ID3v1.Text != "")
                            Year_Box.Text = YearBox_ID3v1.Text;
                        else
                            Year_Box.Text = YearBox_ID3v2.Text;
                    ...
                    else
                    {
                        ...
                        {
                            case "TITLE":
                                Title_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "ALBUM":
                                Album_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "ARTIST":
                                Artist_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "TRACKNUMBER":
                                Track_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "DATE":
                                Year_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "COMMENTS":
                                Comment_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                            case "GENRE":
                                Genre_Box.Text = Marshal.PtrToStringAnsi(tag.data); break;
                        }
                    }
                }
            }
        ...
        private string Genre_Determination(string number)
        {
            byte Genre_byte = Convert.ToByte(number);
            if (Genre_byte == 255)
                return "";
            else
            {
                switch (Genre_byte)
                {
                    case 00: return "Blues";
                    case 01: return "Classic Rock";
                    case 02: return "Country";
                    case 03: return "Dance";
                    case 04: return "Disco";
                    case 05: return "Funk";
                    case 06: return "Grunge";
                    case 07: return "Hip-Hop";
                    case 08: return "Jazz";
                    case 09: return "Metal";
        ...
        private void Song_Info_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
        }
    }
